...
const EarthquakeDetail = () => {
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center p-6">
          <AlertTriangle className="mx-auto h-12 w-12 text-mint mb-4" />
          <h1 className="text-2xl font-bold text-forest mb-2">Earthquake not found</h1>
          <p className="text-forest/70 mb-6">The earthquake details you're looking for could not be found.</p>
          <Link to="/">
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <Link to="/" className="inline-flex items-center text-forest hover:text-forest-light mb-6">
        <ArrowLeft className="mr-2 h-4 w-4" />
        Back to Dashboard
      </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-white p-6 rounded-lg shadow-md">
          <div className="flex justify-between items-start">
            <div>
              ...
              <div className="flex items-center text-forest/70 mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-forest/10 p-4 rounded-lg">
              <div className="text-sm text-forest/70">Magnitude</div>
              <div className={`text-2xl font-bold ${earthquake.magnitude >= 5 ? 'text-red-500' : 'text-forest'}`}>
                {earthquake.magnitude.toFixed(1)}
              </div>
            </div>
            <div className="bg-forest/10 p-4 rounded-lg">
              <div className="text-sm text-forest/70">Depth</div>
              <div className="text-2xl font-bold text-forest">
                {earthquake.depth.toFixed(1)} km
              </div>
            </div>
            <div className="bg-forest/10 p-4 rounded-lg">
              <div className="text-sm text-forest/70">Source</div>
              <div className="text-2xl font-bold text-forest">
          ...
          <div className="mb-6">
            <h2 className="text-xl font-semibold text-forest mb-3">Event Timeline</h2>
          ...
          <div className="mb-6">
            <h2 className="text-xl font-semibold text-forest mb-3">Seismic Activity Graph</h2>
            <div className="h-64 bg-forest/5 p-4 rounded-lg">
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-xl font-semibold text-forest mb-3">Technical Details</h2>
            <Table>
              <TableBody>
                <TableRow>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-xl font-semibold text-forest mb-3">Risk Assessment</h2>
            <div className={`p-4 rounded-lg ${
              earthquake.magnitude >= 6.0 ? 'bg-red-50 border border-red-200' : 
              earthquake.magnitude >= 4.5 ? 'bg-yellow-50 border border-yellow-200' : 
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-xl font-semibold text-forest mb-3">Nearby Events</h2>
              ...
              {earthquakes?.filter(eq => eq.id !== earthquake.id)
                ...
                .map(eq => (
                  <Link 
                    key={eq.id} 
                    to={`/earthquake/${eq.id}`}
                    className="block p-3 border border-forest/20 rounded-lg hover:bg-forest/5"
                  >
                    <div className="flex justify-between">
                      <span className="font-medium">{eq.place}</span>
                      <span className={`${eq.magnitude >= 5 ? 'text-red-500' : 'text-forest'}`}>
